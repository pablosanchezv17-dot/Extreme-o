...
export default function RegistroCliente() {
  ...
  async function manejarSubmit(e: React.FormEvent) {
    ...
    const login = await signIn("credentials-usuario", { redirect: false, email, password });
    setCargando(false);
    if (login?.ok) {
      router.push("/cuenta");
      router.refresh();
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-card space-y-4">
          ...
          <p className="text-center font-body text-sm text-neutral-500">
            ¿Ya tienes cuenta?{" "}
            <Link href="/cuenta/login" className="font-semibold text-olive-700 hover:underline">
              Inicia sesión
            </Link>
          </p>
        </div>
